...
class SpecializationController extends Controller
{
    public function viewServices(Request $request)
    {
        ...
        $topSpecializations = Specialization::whereNull('parent_id')->get();
        return view('settingsSpecialization', compact('topSpecializations','user'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $specialization->added_by = 1;
        // $specialization->added_by = Auth::user()->id;
        $specialization->ip_address = $request->ip();
        $specialization->save();
    ...
    public function specializationActionFunc(Request $request)
    {
        $message = "";
        $currentServiceId = $request->get('currentServiceId');
        log::info($currentServiceId);
        $currentAction = $request->get('currentAction');
        $service = Specialization::find($currentServiceId);
        if (!$service) {
    ...
    public function specializationDeleteFunction(Request $request)
    {
        $message = "";
        $currentServiceId = $request->get('currentServiceId');
        $currentAction = $request->get('currentAction');
        if ($currentAction == 'delete') {
            $service_del = Specialization::find($currentServiceId)->delete();
            if (!$service_del) {
    ...
    public function getSpecializationData(Request $request)
    {
        $table = Constants::DB_PREFIX . '_specialization_master';
